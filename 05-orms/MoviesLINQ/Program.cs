...
var count = movies.Count(m => m.Genre == "Comedy");
var comedies = movies.Where((m) => m.Genre == "Comedy").ToList();

Console.WriteLine($"Comedy Count: {count}");
PrintEach(comedies, "Comedy Movies");

// Find all movies rated higher than 9.0

var highRatedMovies = movies.Where(m => m.Rating > 9.0).ToList();
PrintEach(highRatedMovies, "High Rated Movies");

var badMovies = movies.Where(m => m.Rating < 4.0).ToList();
PrintEach(badMovies, "Bad Movies");

var sortByReleaseDate = movies.OrderByDescending(m => m.ReleaseDate).ToList();
PrintEach(sortByReleaseDate, "Movies Sorted by Release Date");

// List of titles of each movie

var titles = movies.Select(m => m.Title).ToList();
PrintEach(titles, "Movie Titles");

var groupedByGenre = movies.GroupBy(m => m.Genre).ToList();

foreach (var genre in groupedByGenre)
{
    Console.WriteLine($"\n{genre.Key}");
